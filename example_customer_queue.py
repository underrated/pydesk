...
class Customer(Component):
	...
	def main_thread(self):
		...
		print "Time:"+str(sim.time)+" : Customer "+self.name+" is going to the store..."
		yield wait_time,self.arrival_time
		self.parent.register_customer(self)
		print "Time:"+str(sim.time)+" : Customer "+self.name+" arrived at the store and is waiting in line..."
		yield wait_event,self.served
		self.done = True
		print "Time:"+str(sim.time)+" : Customer "+self.name+" has been served and left the store..."
...
class Store(Component):
	...
	def serve_customer(self):
		...
		print "Time:"+str(sim.time)+" : Store : Serving customer "+c.name
		c.served.emit() # RULE : emit event before context switch
		yield wait_time,1
		print "Time:"+str(sim.time)+" : Store : Served customer "+c.name
	
	def main_thread(self):
		while True:
			if len(self.customers)!=0:
				#print "Time:"+str(sim.time)+" : Store : We have customers..."
				yield wait_thread,self.serve_customer()
			else:
				#print "Time:"+str(sim.time)+" : Store : No customers in line yet, waiting..."
				yield wait_time,1
...
class Top(Component):
	...
	def main_thread(self):
		while True:
			...
			if False in states:
				#print  "Time:"+str(sim.time)+" : Top : We have unserved customers, waiting..."
				yield wait_time,1
			else:
				print  "Time:"+str(sim.time)+" : Top : All customers have been served, stopping simulation"
